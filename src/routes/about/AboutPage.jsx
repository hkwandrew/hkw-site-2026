...
import { CLOUDS, QUOTES } from './about-clouds-quotes'
import MarmotCheer from './MarmotCheer'
import useAboutPageAnimation from './useAboutPageAnimation'
import {
    AboutPageStyle,
    AnimatedSection,
    Cloud,
...
const HeroCloudSvg = ({ includeMorphTarget = false }) => (
...
const AboutPage = () => {
    ...
    const renderQuoteBody = (quote, components) => {
        const Text = components.TextComponent
        const Name = components.NameComponent
        const Role = components.RoleComponent

        return (
            <>
                <Text>{quoteTextWithMarks(quote.quote)}</Text>
            ...
            <AnimatedSection className='about about--animated' id='about' ref={sectionRef}>
                <Scene className='about-scene'>
                    <HeroCloud className='about-heroCloud' aria-hidden='true'>
                        <HeroCloudSvg includeMorphTarget />
                    </HeroCloud>
                    ...
                    <CloudsLayer aria-hidden='true'>
                        {CLOUDS.map((cloud) => {
                            const CloudArt = cloud.component
                    ...
                    <QuotesLayer>
                        {QUOTES.map((quote) => (
                            ...
                            >
                                {renderQuoteBody(quote, {
                                    NameComponent: QuoteName,
                                    RoleComponent: QuoteRole,
                                    TextComponent: QuoteText,
                                })}
                            </Quote>
                        ))}
                    </QuotesLayer>
            ...
            <StaticSection aria-label='Client testimonials'>
                ...
                <StaticCards>
                    {MOBILE_CARDS.map((quote, index) => {
                        ...
                        const CardCloud = cardCloud?.component
                            ...
                            >
                                ...
                                <StaticCardContent>
                                    {quote.isOutro ? (
                                        ...
                                        renderQuoteBody(quote, {
                                            NameComponent: StaticQuoteName,
                                            RoleComponent: StaticQuoteRole,
                                            TextComponent: StaticQuoteText,
                                        })
                                    )}
                                </StaticCardContent>
